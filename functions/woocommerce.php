...
function flexee_replace_default_thumbnail() {
...
add_action( 'init', 'flexee_replace_default_thumbnail' ); 

// Remove select2 
function flexee_manage_woocommerce_styles() {
	wp_dequeue_script( 'select2' ); //They don't use Chosen anymore, it was replaced with select2 in WC 2.3
}
add_action( 'wp_enqueue_scripts', 'flexee_manage_woocommerce_styles', 99 );

// Remove cart buttons on shop/archive/category pages 
function flexee_remove_add_to_cart_buttons() {
	remove_action( 'woocommerce_after_shop_loop_item', 'woocommerce_template_loop_add_to_cart' );
}
add_action( 'woocommerce_after_shop_loop_item', 'flexee_remove_add_to_cart_buttons', 1 ); 
...
function flexee_remove_woocommerce_product_tabs( $tabs ) {
	unset( $tabs['description'] );
	unset( $tabs['reviews'] );
	unset( $tabs['additional_information'] );
	return $tabs;
}
add_filter( 'woocommerce_product_tabs', 'flexee_remove_woocommerce_product_tabs', 98 );
